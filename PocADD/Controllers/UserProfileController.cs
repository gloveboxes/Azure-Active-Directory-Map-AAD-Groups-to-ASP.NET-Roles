...
namespace PocADD.Controllers
{
    [Authorize(Roles = "Doctors, Admins")]
        ...
        public async Task<ActionResult> Index()
        {
            ...
            try
            {
                ...
                foreach (Claim claim in ClaimsPrincipal.Current.FindAll("groups"))
                {
                    try
                    {
                        var go = await activeDirectoryClient.Groups
                            .Where(u => u.ObjectId.Equals(claim.Value))
                            .ExecuteAsync();

                        string displayName = go.CurrentPage.ToList().First().DisplayName;
                    }
                    catch (Exception ex) { }
                }
